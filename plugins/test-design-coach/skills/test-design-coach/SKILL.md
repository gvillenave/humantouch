...
When the units are covered (or the user says they are done), reconcile the session against your risk map from step 1: every risk you mapped ends as a named case (written or implemented), an explicit decline by the user, something you already dismissed as not worth testing in step 2, or a gap you now name — checked against the actual tests and the user's actual verdicts, not recalled. A risk that quietly fell out of the conversation gets raised as a question, and the recap waits for the answer. Then recap briefly: what is now protected, what the user declined, and any assumption worth revisiting when the code changes. Mention untested areas plainly; a known gap is fine, an unknown one is not.
